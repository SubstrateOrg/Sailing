...
use support::{StorageValue, StorageMap, dispatch::Result, decl_module, decl_storage, decl_event, ensure, Parameter};
use system::ensure_signed;
use sr_primitives::traits::{ SimpleArithmetic, Member, Bounded, Zero};
use rstd::prelude::*;
...
pub trait Trait: balances::Trait {
    ...
    type TokenIndex: Parameter + Member + SimpleArithmetic + Bounded + Default + Copy;
}
/*
type TokenLinkedItem<T> = LinkedItem<<T as Trait>::TokenIndex>;
type OwnedTokensList<T> = LinkedList<OwnedTokens<T>, <T as system::Trait>::AccountId, <T as Trait>::TokenIndex>;
*/
...
decl_event!(
    ...
    where
        ...
        <T as self::Trait>::TokenIndex,
    {
        Transfer(Option<AccountId>, Option<AccountId>, TokenIndex),
        Approval(AccountId, AccountId, TokenIndex),
...
decl_storage! {
    trait Store for Module<T: Trait> as ERC721Storage {
        ...
        pub TokenOwner get(owner_of): map T::TokenIndex => Option<T::AccountId>;
        pub TokenApprovals get(get_approved): map T::TokenIndex => Option<T::AccountId>;
        ...
        pub OwnedTokens get(token_of_owner_by_index): map (T::AccountId, u64) => T::TokenIndex;
        pub OwnedTokensIndex: map T::TokenIndex => u64;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn approve(origin, to: T::AccountId, token_id: T::TokenIndex) -> Result {
        ...
        fn transfer_from(origin, from: T::AccountId, to: T::AccountId, token_id: T::TokenIndex) -> Result {
            let sender = ensure_signed(origin)?;
        ...
        fn safe_transfer_from(origin, from: T::AccountId, to: T::AccountId, token_id: T::TokenIndex) -> Result {
        ...
        fn create_token(origin) -> Result {
            ...
            Self::_mint(sender, Self::_next_token_id().unwrap())?;
...
impl<T: Trait> Module<T> {
    ...
    pub fn _next_token_id() -> result::Result<T::TokenIndex, &'static str> {
		let token_id = Self::total_supply() + T::TokenIndex::from(1u32);
		if token_id == T::TokenIndex::max_value() {
			return Err("Tokens count overflow");
		}
		Ok(token_id)
	}
    ...
    pub fn _mint(to: T::AccountId, token_id: T::TokenIndex) -> Result {
    ...
    pub fn _burn(token_id: T::TokenIndex) -> Result {
        ...
        // Writing to storage begins here
        Self::_remove_token_from_all_tokens_enumeration()?;
    ...
    pub fn _add_token_to_all_tokens_enumeration() -> Result {
        ...
        let new_total_supply =  total_supply + T::TokenIndex::from(1u32);
    ...
    pub fn _remove_token_from_all_tokens_enumeration() -> Result {
        ...
        let new_total_supply =  total_supply - T::TokenIndex::from(1);
